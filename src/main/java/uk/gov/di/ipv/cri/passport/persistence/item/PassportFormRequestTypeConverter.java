...
public class PassportFormRequestTypeConverter implements AttributeConverter<PassportFormRequest> {
    ...
    public AttributeValue transformFrom(PassportFormRequest input) {
        ...
        Map<String, AttributeValue> attributeValueMap = Map.of(
                ...
                "forenames", AttributeValue.builder().s(Arrays.toString(input.forenames)).build(),
                "dateOfBirth", AttributeValue.builder().s(input.dateOfBirth.toString()).build(),
                "expiryDate", AttributeValue.builder().s(input.expiryDate.toString()).build()
        );
