...
Feature: Passport CRI API
  ...
  Scenario: Acquire initial JWT and Passport Happy path
    Given Passport user has the user identity in the form of a signed JWT string for CRI Id passport-cri-shared-dev and row number 6
    ...
    And Passport user sends a POST request to Access Token endpoint passport-cri-shared-dev
  ...
  ### Duplicate tests required to avoid revoked token when trying to reuse
  @intialJWT_happy_path @passportCRI_API @pre-merge @dev
  Scenario: Acquire initial JWT and Passport Happy path
    Given Passport user has the user identity in the form of a signed JWT string for CRI Id passport-cri-shared-dev and row number 6
    And Passport user sends a POST request to session endpoint
